...
metadata:
  pattern: composite
  components:
    - pipeline: 四阶段流水线（内容编写 → 网页开发 → 音频合成 → 录屏）带三个硬性检查点（Checkpoint Plan / 第1章验收 / Checkpoint Audio）
    - generator: Vite+Vue3 项目脚手架，按主题 token 生成可运行代码骨架
  depends_on:
    - references/SCRIPT-STYLE.md   # 文章→口播稿风格指南
    - references/OUTLINE-FORMAT.md  # outline.md 格式规范
    - references/CHAPTER-CRAFT.md   # 章节实现单一入口（原则/决策树/反模式/自检）
    - references/AUDIO.md           # 音频合成流程
    - references/RECORDING.md       # 录屏与后期
    - references/THEMES.md          # 主题系统完整契约
    - references/adk-patterns.md    # （or-skill-crafter 引用，如存在）
    - templates/                    # Vite+Vue3 脚手架模板
    - themes/                       # 内置主题集
    - scripts/scaffold.sh           # 一键脚手架脚本
---

# Web Video Presentation

把一篇文章或口播稿，一步步做成可录屏的"伪装成视频的网页"，可选合成
口播音频。产出物 = Vite + Vue 3 + TS 项目 + 按章节切分的音频。
...
└── presentation/       # 脚手架产出的 Vite + Vue 3 + TS 项目
    ├── src/chapters/<NN>-<id>/
    │   ├── <Chapter>.vue     # 视觉实现
    │   ├── <Chapter>.css
...
> 章节 `.vue` 里的 `v-if="step === N"` 出现的最大 N + 1 必须等于
...
| 3 | 每步独占整屏 | `v-if="step === N"` → `<FullScene />` |

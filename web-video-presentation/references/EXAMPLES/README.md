...
| 例子 | 适用场景 | 文件 |
|---|---|---|
| [`hook-chapter/`](hook-chapter/) | **钩子型开场** —— 多张图片逐张揭示后 hero takeover | `chapter.vue` + `chapter.css` |
| [`list-reveal/`](list-reveal/) | **列举型** —— 口播说"三件事 / N 个特性"，每项 1 step | `chapter.vue` + `chapter.css` |
...
3. 翻 `chapter.vue` 看实现：模板结构、`step` 切分、用了哪些组件 / 类名

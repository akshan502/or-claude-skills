...
echo "▸ 在 $TARGET 创建 Vite + Vue 3 + TS 项目"
echo "▸ 使用主题：$THEME"
npm create vite@latest "$TARGET" -- --template vue-ts >/dev/null
...
cp "$TEMPLATES/src/main.ts" src/main.ts
cp "$TEMPLATES/src/App.vue"  src/App.vue
...
cp "$TEMPLATES/src/components/Stage.vue"          src/components/Stage.vue
cp "$TEMPLATES/src/components/MaskReveal.vue"     src/components/MaskReveal.vue
cp "$TEMPLATES/src/components/ProgressBar.vue"    src/components/ProgressBar.vue
cp "$TEMPLATES/src/components/ProgressBar.css"    src/components/ProgressBar.css
cp "$TEMPLATES/src/components/AutoStartGate.vue"  src/components/AutoStartGate.vue
cp "$TEMPLATES/src/components/AutoStartGate.css"  src/components/AutoStartGate.css
cp "$TEMPLATES/src/components/AutoToggle.vue"     src/components/AutoToggle.vue
...
cp "$TEMPLATES/src/chapters/01-example/Example.vue"     src/chapters/01-example/Example.vue
...
然后：
  ...
  • **每章必须有 narrations.ts**（与 Example.vue 同目录），
